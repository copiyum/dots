...
if [[ "${TERM_PROGRAM:-}" == "iTerm.app" || "${TERM_PROGRAM:-}" == "iTerm2" ]]; then
  if [[ -x /Applications/iTerm.app/Contents/Resources/it2profile ]]; then
    /Applications/iTerm.app/Contents/Resources/it2profile -s "macOS Rice White" >/dev/null 2>&1 || true
  else
    printf '\033]1337;SetProfile=macOS Rice White\a'
  fi
fi
...
export POSH_THEME="$HOME/.config/oh-my-posh/rice-white.omp.json"
